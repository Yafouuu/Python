...
def test_total_animaux():
    ...
    assert troupeaux.total_animaux(mon_troupeau) == 56
...
def test_tous_les_animaux():
    ...
    assert troupeaux.tous_les_animaux(mon_troupeau) == {'mouton','brebis','chien'}
...
def test_le_plus_represente():
    ...
    assert troupeaux.le_plus_represente(mon_troupeau) == 'mouton'
...
def test_reunion_troupeaux():
    ...
    assert troupeaux.reunion_troupeaux(troupeau_de_perrette, mon_troupeau) == {'veau':14, 'vache':7, 'poule':42,'mouton': 45, 'brebis': 4, 'chien' :7}
    assert troupeaux.reunion_troupeaux(mon_troupeau, troupeau_de_jean) == {'mouton': 45, 'brebis': 4, 'chien' :7,'vache':12, 'cochon':17, 'veau':3}
    assert troupeau_de_jean == {'vache':12, 'cochon':17, 'veau':3}
    assert troupeau_de_perrette == {'veau':14, 'vache':7, 'poule':42}
